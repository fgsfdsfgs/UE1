...
public:
	...
	{
		iX = Clamp(appRound( (Location.X + XY_OFS) * (1.0/GRAN_XY) ), 0, (int)NUM_BUCKETS-1);
		iY = Clamp(appRound( (Location.Y + XY_OFS) * (1.0/GRAN_XY) ), 0, (int)NUM_BUCKETS-1);
		iZ = Clamp(appRound( (Location.Z + Z_OFS ) * (1.0/GRAN_Z ) ), 0, (int)NUM_BUCKETS-1);
	}
